...
def generated_audio_callback(samples: np.ndarray, progress: float):
    samples = resampy.resample(samples, 22050, 44100, axis=0)
    buffer.put(samples)
...
def play_audio():
    with sd.OutputStream(channels=1, callback=play_audio_callback, dtype="float32", samplerate=22050*2, blocksize=1024,):
        event.wait()
        event.clear()
...
def play_text_audio(text, tts):
    ...
    if len(audio.samples) != 0:
        sf.write("./generated.wav", audio.samples, samplerate=22050, subtype="PCM_16",)
        ...
        tts_config = sherpa_onnx.OfflineTtsConfig(
            model=sherpa_onnx.OfflineTtsModelConfig(
                vits=sherpa_onnx.OfflineTtsVitsModelConfig(
                    model='/root/Voice_Assistant/model/tts/theresa.onnx',
                    lexicon='/root/Voice_Assistant/model/tts/lexicon.txt',
                    data_dir="",
                    tokens="/root/Voice_Assistant/model/tts/tokens.txt",
                ),
                provider="cpu",
                debug=False,
                num_threads=1,
